...
function getFolder(kind: string) {
...
export async function POST(request: Request) {
  try {
    ...
    const folder = typeof kindEntry === "string" ? getFolder(kindEntry) : null;

    if (!folder) {
      return NextResponse.json(
        { success: false, error: "Type d'image invalide." },
        { status: 400 }
      );
    }
    ...
    const storagePath = `${tenantId}/${folder}/${safeName}`;
